...
<?php global $post, $posts, $query_string, $wp_query, $s; ?>
	...
	<div class="listing-page">
		...
		<?php if( $custom_query->have_posts() ) : ?>
			...
			<?php if( $gantry->get( $page_context . '-page-heading-enabled', '1' ) ) : ?>
				...
				<?php else : ?>
					...
					<h1>
						<?php printf( __( 'Search Results for: %s', 'rt_cygnet_wp_lang' ), '<span>' . get_search_query() . '</span>' ); ?>
					</h1>
		...
		<?php else : ?>
			...
			<h1>
				<?php _e( 'Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'rt_cygnet_wp_lang' ); ?>
			</h1>
