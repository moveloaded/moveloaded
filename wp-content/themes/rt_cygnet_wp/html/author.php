		...
		<?php if( $custom_query->have_posts() ) : ?>
			...
			<?php if( $gantry->get( $page_context . '-page-heading-enabled', '1' ) ) : ?>
				...
				<?php else : ?>
					...
					<h1>
						<?php printf( __( 'Author Archives: %s', 'rt_cygnet_wp_lang' ), '<span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( "ID" ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' ); ?>
					</h1>
			...
			<?php if( get_the_author_meta( 'description' ) ) : ?>
				...
				<div class="author-info">
					<div class="author-name">
						<h2>
							<?php printf( __( 'About %s', 'rt_cygnet_wp_lang' ), get_the_author() ); ?>
						</h2>
					</div>
