...
<?php global $post, $posts, $query_string, $wp_query; ?>
	...
	<div class="listing-page">
			...
			<?php if( $gantry->get( $page_context . '-page-heading-enabled', '1' ) ) : ?>
				...
				<?php else : ?>
					...
					<h1>
						<?php
							if ( is_day() ) :
								printf( __( 'Daily Archives: %s', 'rt_cygnet_wp_lang' ), '<span>' . get_the_date() . '</span>' );
							elseif ( is_month() ) :
								printf( __( 'Monthly Archives: %s', 'rt_cygnet_wp_lang' ), '<span>' . get_the_date( __( 'F Y', 'monthly archives date format' ) ) . '</span>' );
							elseif ( is_year() ) :
								printf( __( 'Yearly Archives: %s', 'rt_cygnet_wp_lang' ), '<span>' . get_the_date( __( 'Y', 'yearly archives date format' ) ) . '</span>' );
							else :
								_e( 'Archives', 'rt_cygnet_wp_lang' );
							endif;
						?>
					</h1>
		...
		<?php else : ?>
			...
			<h1>
				<?php _e( "Sorry, but there aren't any posts matching your query.", 'rt_cygnet_wp_lang' ); ?>
			</h1>
