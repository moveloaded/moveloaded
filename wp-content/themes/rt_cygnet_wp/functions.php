...
function gantry_admin_missing_nag() {
	$msg = __( 'The active theme requires the Gantry Framework Plugin to be installed and active', 'rt_cygnet_wp_lang' );
...
function gantry_missing_nag() {
	echo __( 'This theme requires the Gantry Framework Plugin to be installed and active.', 'rt_cygnet_wp_lang' );
	die(0);
}
...
function gantry_pagination($custom_query) {
	...
	$pagination = paginate_links( array(
		...
		'type' => 'list',
		'next_text' => __( 'Next', 'rt_cygnet_wp_lang' ),
		'prev_text' => __( 'Previous', 'rt_cygnet_wp_lang' )
	) );
	...
	<div class="pagination">
		...
		<p class="counter">
			<?php printf( __( 'Page %1$s of %2$s', 'rt_cygnet_wp_lang' ), $current_page, $custom_query->max_num_pages ); ?>
		</p>
		<?php endif; ?>
