	...
	this.Marquee = new Class({
		...
		initialize: function(options){
			...
			this.overlay = new Element('div',{
				'styles': { 'position': 'relative', 'background': 'url(' + RokGallerySettings.images + 'blank.gif)', 'height': '100%', 'width': '100%',   'z-index': this.options.zindex+1 }
			}).inject(this.box);
			...
			['left','right','top','bottom'].each(function(side,idx){
				switch(side){
					case 'left': style = Object.merge(Object.clone(antStyles),{top: 0, left: -1, height: '100%'}); break;
					case 'right': style = Object.merge(Object.clone(antStyles),{top: 0, right: -1, height: '100%'}); break;
					case 'top': style = Object.merge(Object.clone(antStyles),{top: -1, left: 0, width: '100%'}); break;
					case 'bottom': style = Object.merge(Object.clone(antStyles),{bottom: -1, left: 0, width: '100%'}); break;
				}
			...
			document.body.onselectstart = function(e){ new Event(e).stop(); return false; };

			// better alternative?
			this.removeDOMSelection = (document.selection && document.selection.empty) ? function(){ document.selection.empty(); }: 
				(window.getSelection) ? function(){ var s=window.getSelection();if(s && s.removeAllRanges) s.removeAllRanges();}: Function.from(false);
		...
		refresh: function(fast){
			...
			if (!fast) this.fireEvent('resize',this.getRelativeCoords());
		},
		...
		getRelativeCoords: function(){
			var box = this.coords.box, cc = Object.merge(Object.clone(this.coords.container)), c = this.coords;
			if(!this.options.contain) cc = { x: [0,0], y: [0,0]};
			return { 
				x: ((box.x[0] - cc.x[0]).toInt()).limit(0, box.x[0] - cc.x[0]), 
				y: ((box.y[0] - cc.y[0]).toInt()).limit(0, box.y[0] - cc.y[0]), 
				w: (c.w).toInt(), 
				h: (c.h).toInt() 
			};
		},
