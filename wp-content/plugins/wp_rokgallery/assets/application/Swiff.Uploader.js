...
Swiff.Uploader = new Class({
	...
	reposition: function(coords) {
		...
		if (Browser.ie8 || Browser.ie9){
			for (var coord in coords){
				if (isNaN(coords[coord])) coords[coord] = 0;
			}
		}	
	...
	processFiles: function(successraw, failraw, queue) {
		...
		if (successraw) {
			if (this.fileList.length >= 100 || this.fileList.length + successraw.length > 100){
				var red = '#ed2e26';
			...
			if (this.fileList.length >= 100) successraw.empty();
			else if (this.fileList.length + successraw.length > 100){
				var fill = 100 - this.fileList.length;
				successraw = successraw.slice(0, fill);
			}
			...
			if (successraw.length) this.fireEvent('selectSuccess', [success], 10);
		}
		...
		if (successraw.length) this.update(queue);
		
		window.Popup.counter.getElement('span').set('text', this.fileList.length);
